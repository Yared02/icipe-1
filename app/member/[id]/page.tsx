...
const DetailPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchParticipants = async () => {
      ...
      const { data, error } = await supabase
        ...
        .select("*")
        .eq("group_id", id); // Replace "id" with the actual column name you want to filter by
  ...
  // Function to remove the selected participant from the group in Supabase
  const removeSelectedParticipants = async () => {
    for (const participantId of selectedParticipants) {
      ...
      if (error) {
        console.error("Error deleting participant from group:", error.message);
      }
    }

    alert("Selected participants removed successfully!");
    // Refetch participants after deletion
    setParticipants((prev) =>
      prev.filter((participant) => !selectedParticipants.includes(participant.id))
    );
    setSelectedParticipants([]); // Clear selected participants after deletion
  };
  ...
  return (
    <div>
      ...
      {/* Button to remove selected participants */}
      <button
